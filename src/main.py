    ...
    try:
        ...
        if extract:
            ...
            if extract_to == "":
                ...
                os.chdir(extract_to)
                os.system(f'unzip {path}')
            if Path(str(path).replace(".zip", "")).exists() or Path(
                    f'{extract_to}/{str(path.name).replace(".zip", "")}').exists():
                path = Path(str(path).replace(".zip", "")) if extract_to == '' else Path(
                    f'{extract_to}/{str(path.name).replace(".zip", "")}')
            else:
